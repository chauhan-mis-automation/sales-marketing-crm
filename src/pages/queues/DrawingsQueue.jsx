...
export default function DrawingsQueue() {
  ...
  async function loadData() {
    ...
    const { data: gaRows } = await supabase.from('ga_drawing_tasks').select('*')
    const { data: enqRows } = await supabase.from('enquiries').select('*').eq('status', 'Active')

    const enqMap = {}
    ;(enqRows || []).forEach(e => { enqMap[e.enquiry_id] = e })

    let pending = latestPerEnquiry(gaRows).filter(t => t.status !== 'Client Approved' && enqMap[t.enquiry_id])
  ...
  return (
    <div className="qp-wrap">
      <p className="qp-subtitle">Enquiries with pending GA Drawing work</p>

      <div className="qp-card">
        <div className="qp-card-header">
          <div className="qp-card-title">📐 GA Drawing Queue <span className="qp-count-sm">({filteredRows.length})</span></div>
          <div className="qp-search">
            <i className="fas fa-search"></i>
            <input placeholder="Search company, contact, designer, ID…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={8}>
                  <div className="qp-empty">
                    ...
                    <p>{rows.length === 0 ? 'No GA Drawing work pending! 🎉' : 'No matches found'}</p>
                  </div>
                </td></tr>
              ) : (
